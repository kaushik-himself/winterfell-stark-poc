...
fn get_all_tokens() -> Vec<i32> {
  ...
  let res = stmt
    .query_map([], |row| Ok(Token { id: row.get(0)? }))
    .unwrap();
  ...
  assert!(
    tokens.len() % 2 == 0,
    "Even number of tokens needed. Mint one more"
  );
  tokens.sort();
...
pub fn generate_proof(token_id: usize) {
  ...
  let base_tokens: Vec<BaseElement> = tokens
    .iter()
    .map(|t| BaseElement::new(*t as u128))
    .collect();
  ...
  let token_index = if token_id % 2 == 0 {
    token_id - 2
  } else {
    token_id - 1
  };
  ...
  let merkle_tree = build_merkle_tree(tree_depth, base_tokens);
  ...
  // Regular proof
  assert!(MerkleTree::<Rescue128>::verify(*merkle_tree.root(), index, &proof).is_ok());
  println!("Verified the non-stark merkle proof");
  assert!(MerkleTree::<Rescue128>::verify(*merkle_tree.root(), index + 1, &proof).is_err());
  ...
  println!(
    "Generated a stark proof for the Merkle Tree, leaf index {}",
    index
  );
